...
void RFM69_ATC::sendFrame(uint8_t toAddress, const void* buffer, uint8_t bufferSize, bool requestACK, bool sendACK, bool sendRSSI, int16_t lastRSSI) {
  ...
  if (sendACK) {                   // TomWS1: adding logic to return ACK_RSSI if requested
    SPI_Transfer(RFM69_CTL_SENDACK | (sendRSSI?RFM69_CTL_RESERVE1:0));  // TomWS1  TODO: Replace with EXT1
    if (sendRSSI) {
    ...
    SPI_Transfer(_targetRSSI ? RFM69_CTL_REQACK | RFM69_CTL_RESERVE1 : RFM69_CTL_REQACK);
  }
  else SPI_Transfer(0x00);
...
void RFM69_ATC::interruptHook(uint8_t CTLbyte) {
  ...
  if (ACK_RECEIVED && ACK_RSSI_REQUESTED) {
    ...
    if (DATALEN >= 1) {
      _ackRSSI = -1 * SPI_Transfer(0); //rssi was sent as single byte positive value, get the real value by * -1
...
//=============================================================================
uint8_t RFM69_ATC::setLNA(uint8_t newReg) {  // TomWS1: New method used to disable LNA AGC for testing purposes
  uint8_t oldReg;
